...
//    MessageBox messageBox = new MessageBox();
    ...
    public void initialize() {
        ...
        goToLoginLink.setOnAction(event -> {
            try {
                FXMLLoader loader = new javafx.fxml.FXMLLoader(getClass().getResource("LoginPage.fxml"));
...
//            userNameField.getText().trim().isEmpty() ||
            ...
            MessageBox.showError("Missing Information", "Please fill in all fields before requesting OTP.");
            return false;
        }

        if (accountTypeGroup.getSelectedToggle() == null) {
            MessageBox.showError("Missing Selection", "Please select account type.");
            return false;
        }

        if (householdRadio.isSelected()) {
            MessageBox.showError("Household Registration", "Please fill in household details.");
            return !(address.getText().trim().isEmpty() && city.getText().trim().isEmpty());
        } else if (workerRadio.isSelected()) {
            MessageBox.showError("Worker Registration", "Please fill in worker details.");
